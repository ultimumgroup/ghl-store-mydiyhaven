...
with sync_playwright() as p:
 ...
 assert page.goto('http://127.0.0.1:4317/store-diagnostics',wait_until='networkidle',timeout=120000).status==200
 data=json.loads(page.locator('pre').first.inner_text())
 page.goto('http://127.0.0.1:4317/products',wait_until='networkidle',timeout=120000)
 href=page.locator('a[href^="/products/"]').first.get_attribute('href')
 assert href
 page.goto('http://127.0.0.1:4317'+href,wait_until='networkidle',timeout=120000)
 ...
 static=plain.new_page();static.goto('http://127.0.0.1:4317'+href,wait_until='domcontentloaded',timeout=120000)
